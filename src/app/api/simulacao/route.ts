...
import { getSimulacoes, salvarSimulacao } from "@/services/simulacao.service";
...
export async function GET(request: Request) {
  ...
  const simulacoes = await getSimulacoes(usuarioId, partidaId ? Number(partidaId) : undefined);
...
export async function POST(request: Request) {
  ...
  try {
    const result = await salvarSimulacao(usuarioId, partidaId, golsMandante, golsVisitante);
