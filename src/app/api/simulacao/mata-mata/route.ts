...
import { getSimulacoesMataMata, salvarSimulacaoMataMata } from "@/services/simulacao.service";
...
export async function GET(request: Request) {
  ...
  const simulacoes = await getSimulacoesMataMata(
    usuarioId,
...
export async function POST(request: Request) {
  ...
  try {
    const result = await salvarSimulacaoMataMata(
      usuarioId,
      partidaId,
      golsMandante,
      golsVisitante,
